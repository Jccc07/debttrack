...
function DueBadge({ dueDate, status, isInstallment }: { dueDate: string | Date | null; status: string; isInstallment?: boolean }) {
  if (isInstallment) return <span className="text-xs text-purple-600">installment</span>;
...
function exportCSV(transactions: Transaction[]) {
  const rows = [
    ["Date", "Type", "Person", "Amount", "Interest", "Total", "Due Date", "Installments", "Status"],
    ...transactions.map((t) => [
      formatDate(t.transactionDate), t.type, t.counterparty ?? "",
      Number(t.amount).toFixed(2), Number(t.interestRate).toFixed(2),
      Number(t.endAmount).toFixed(2),
      t.isInstallment ? `${t.installmentMonths} months` : (t.dueDate ? formatDate(t.dueDate) : "No due date"),
      t.isInstallment ? `${t.installmentMethod}` : "—",
      t.status,
    ]),
  ];
  ...
  a.href = url; a.download = `debttrack-${new Date().toISOString().split("T")[0]}.csv`;
...
function TransactionsPageInner() {
  ...
  async function toggleStatus(tx: Transaction) {
    if (tx.isInstallment) { setSelectedTx(tx); return; }
    setTogglingId(tx.id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {transactions.length > 0 && (
            <button onClick={() => exportCSV(transactions)} className="inline-flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
          ...
          <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-xl transition-colors">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select value={filters.type} onChange={(e) => setFilters({ ...filters, type: e.target.value as TransactionFilters["type"] })}
            ...
            <option value="">All types</option><option value="LEND">Lent</option><option value="OWE">Borrowed</option>
          </select>
          <select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value as TransactionFilters["status"] })}
            className="px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-green-500 transition cursor-pointer hover:border-gray-300">
            <option value="">All status</option><option value="UNPAID">Unpaid</option><option value="PAID">Paid</option><option value="OVERDUE">Overdue</option>
          </select>
          <select value={`${filters.sort}-${filters.order}`}
            onChange={(e) => { const parts = e.target.value.split("-"); const order = parts.pop() as "asc"|"desc"; setFilters({ ...filters, sort: parts.join("-"), order }); }}
            className="px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-green-500 transition cursor-pointer hover:border-gray-300">
          ...
          {(filters.type || filters.status || filters.search) && (
            <button onClick={() => setFilters({ sort: "transactionDate", order: "desc", search: "", type: "", status: "" })} className="px-3 py-2 text-sm text-gray-400 hover:text-gray-600 transition-colors">Clear</button>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => setShowForm(true)} className="mt-3 text-green-600 text-sm hover:underline font-medium">Add your first transaction</button>
          </div>
        ) : (
          <>
            <div className="hidden md:grid md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr_1fr] gap-4 px-5 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider border-b border-gray-50">
              <span>Person</span>
              <span className="text-center">Date</span>
              <span className="text-center">Amount</span>
              <span className="text-center">Due / Plan</span>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-gray-50">
                ...
                <div className="flex gap-2">
                  <button disabled={page === 1} onClick={() => setPage(page - 1)} className="px-3 py-1.5 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-40 transition-colors">Previous</button>
                  <button disabled={page === totalPages} onClick={() => setPage(page + 1)} className="px-3 py-1.5 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-40 transition-colors">Next</button>
                </div>
              </div>
            )}
          </>
      ...
      {showForm && (
        <TransactionForm onClose={() => { setShowForm(false); router.replace("/dashboard/transactions"); }} onSaved={() => fetchTransactions()} />
      )}
      {selectedTx && (
        <TransactionDetailsModal transaction={selectedTx} onClose={() => setSelectedTx(null)}
          onUpdated={handleUpdated}
          onDeleted={(id) => { setTransactions((prev) => prev.filter((t) => t.id !== id)); setTotal((prev) => prev - 1); setSelectedTx(null); }} />
      )}
      {deleteTarget && (
        <DeleteConfirmModal counterparty={deleteTarget.counterparty} loading={deleting}
